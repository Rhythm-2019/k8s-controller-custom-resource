...
func NewController(kubeClientSet *kubernetes.Clientset, networkClientSet *clientset.Clientset, networkInformer v1.NetworkInformer) *Controller {
	...
	klog.Infof("setting event handles")
...
func (c *Controller) Run(threadiness int, stopCh <-chan struct{}) error {
	...
	klog.Infof("starting run control loop")

	klog.Infof("waiting to informer sync")
	...
	klog.Infof("started %d worker", threadiness)
...
func (c *Controller) ControlLoop() bool {
	...
	err := func(obj interface{}) error {
		...
		c.workQueue.Forget(obj)
		klog.Infof("Successfully handle key %s", key)
		return nil
